...
if (! function_exists('link_tag')) {
    ...
    function link_tag(
        ...
        string $hreflang = '',
        array $attributes = [] // Nuevo parámetro opcional para atributos adicionales
    ): string {
        // Si $href es un array, extraer los valores
        if (is_array($href)) {
            $rel       = $href['rel'] ?? $rel;
            $type      = $href['type'] ?? $type;
        ...
        // Construir el array de atributos
        $defaultAttributes = [];

        // Atributo href
        if (preg_match('#^([a-z]+:)?//#i', $href) !== 1) {
            $defaultAttributes['href'] = $indexPage ? site_url($href) : slash_item('baseURL') . $href;
        } else {
        ...
        // Atributo hreflang
        if ($hreflang !== '') {
            $defaultAttributes['hreflang'] = $hreflang;
        }
        ...
        // Atributo media
        if ($media !== '') {
            $defaultAttributes['media'] = $media;
        }

        // Atributo title
        if ($title !== '') {
            $defaultAttributes['title'] = $title;
        }

        // Combinar atributos por defecto con los atributos adicionales
